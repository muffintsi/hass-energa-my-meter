...
from .const import (
    ...
    ENERGA_HISTORICAL_DATA_URL,
    ENERGA_MY_METER_LOGIN_URL,
    ENERGA_ACCOUNT_DATA_URL,
)
...
from .errors import (
    ...
    EnergaStatisticsCouldNotBeLoadedError,
    EnergaMyMeterWebsiteError,
)
...
class EnergaWebsiteConnector:
    ...
    def get_first_historical_consumption_for_type(
        self,
        start_date: datetime,
        meter_id: int,
        mode: EnergaStatsModes,
        stat_type: EnergaStatsTypes,
    ) -> datetime | None:
        ...
        try:
            while retries < 10:
                retries += 1
                _LOGGER.debug(
                    "Searching for the first %s statistic in the period %s (%s)",
                    stat_type.value,
                    current_period.strftime("%Y/%m/%d"),
                    int(current_period.timestamp()) * 1000,
                )
                response = self._get_statistic_for_date(
                    current_period.replace(hour=0, minute=0, second=0, microsecond=0),
                    stat_type,
                    meter_id,
                    mode,
                )
                ...
                if len(response.historical_points) == 0:
                    _LOGGER.debug(
                        ...
                        result.strftime("%Y/%m/%d") if result else "None",
                    )
                    break
                ...
                if non_empty:
                    ...
                    _LOGGER.debug(
                        "A new first non-empty %s statistic found for %s.",
                        stat_type.value,
            ...
            _LOGGER.error(
                "Got an error response from the energa website when getting historical stats %s (id: %s): %s",
                ENERGA_HISTORICAL_DATA_URL,
                meter_id,
                error,
            )
    ...
    def _get_statistic_for_date(
        ...
        try:
            ...
            request = mechanize.Request(
                url=ENERGA_HISTORICAL_DATA_URL,
                method="GET",
                data=request_data,
            )
            ...
            _LOGGER.error(
                "Got an error response from the energa website %s (id: %s): %s",
                ENERGA_HISTORICAL_DATA_URL,
                meter_id,
                error,
            )
    ...
    def _prepare_browser() -> Browser:
        ...
        # 🔑 Wyłączamy weryfikację SSL tylko dla tego Browsera
        ctx = ssl._create_unverified_context()
        browser.set_ca_data(context=ctx)
